...
exports.createRun = function (jobId, number, cmds) {
   ...
   var pail;
   if (number) {
       pail = new Pail({ dirPath: internals.Run.settings.dirPath + '/' + jobId + '/pr/' + number });
   } else {
...
exports.cancelRun = function (jobId, number, runId) {

   var config;
   var pids = [];
   var pail;
   if (number) {
       config = internals.Run.getRun(jobId, number, runId);
       pids = internals.Run.getRunPids(jobId, number, runId);
       pail = new Pail({ dirPath: internals.Run.settings.dirPath + '/' + jobId + '/pr/' + number });
   } else {
...
exports.getRun = function (jobId, number, runId) {
...
exports.getRunPids = function (jobId, number, runId) {
...
exports.getRuns = function (jobId, number) {
   ...
   if (number) {
    ...
    for (var i = 0; i < runs.length; i++) {
        var run;
        if (number) {
            run = internals.Run.getRun(jobId, number, runs[i]);
        } else {
...
exports.deleteRun = function (jobId, number, runId) {
...
exports.startRun = function(jobId, number) {

    if (number) {
        if (internals.prRuns[jobId]) {
            console.log('queuing up pr: ' + number + ' for jobId: ' + jobId);
            return null;
        }
    } else {
        if (internals.jobRuns[jobId]) {
    ...
    var workspace;
    if (number) {
        workspace = jobId + '/pr/' + number + '/' + internals.Run.settings.workspace;
        jobPail.createDir(jobId + '/pr');
        jobPail.createDir(jobId + '/pr/' + number);
    } else {
    ...
    var runPail;
    if (number) {
        createConfig = internals.Run.createRun(jobId, number, cmds);
        runPail = new Pail({ dirPath: internals.Run.settings.dirPath + '/' + jobId + '/pr/' + number });
    } else {
    ...
    if (jobConfig.scm) {
       ...
       //if (jobConfig.scm.type === 'git') {
           ...
           var result = bobber.checkoutCode(internals.Run.settings.dirPath + '/' + workspace, jobConfig.scm, null);
           if (result.status === 'failed') {
               ...
               if (jobConfig.scm.runOnCommit) {
                   ...
                   var runs;
                   if (number) {
                       runs = internals.Run.getRuns(jobId, number);
                   } else {
    ...
    internals.runCommands(jobId, number, runConfig.id, commands, internals.onFinish);
    return runConfig.id;
};
...
internals.onFinish = function (jobId, number, runId, err) {
...
//       if (jobConfig.scm.type === 'git') {
          ...
          var workspace;
          if (number) {
              workspace = internals.Run.settings.dirPath + '/' + jobId + '/pr/' + number + '/' + internals.Run.settings.workspace;
          } else {
...
internals.runCommand = function (jobId, number, runId, cmdIndex, cmd, callback) {
...
internals.saveResult = function (jobId, number, runId, cmdIndex, result) {
...
internals.runCommands = function (jobId, number, runId, commands, ready) {
    ...
    function iterate() {
        ...
        if (!nextCommand) {
            //process.chdir(origDir);
            return ready(jobId, number, runId, null);
        }
        if (typeof nextCommand === 'object') {
            ...
            internals.runCommand(jobId, number, runId, cmdIndex, nextCommand, function (result) {
                ...
                internals.saveResult(jobId, number, runId, cmdIndex, result);
                if (result.code !== 0) {
                ...
                if (err) {
                    ...
                    //process.chdir(origDir);
                    return ready(jobId, number, runId, err);
                }
                cmdIndex++;
                return iterate();
            });
