        ...
        if (jobConfig.scm.type === 'git') {
            ...
            bobber.checkoutCode(options, function (result) {
                ...
                if (result.status === 'failed') {
                    ...
                    internals.finishRun(jobId, pr, runConfig.id, 'failed', function () {
                    ...
                    if (jobConfig.scm.runOnCommit) {
                        ...
                        bobber.getLatestCommit(Path.join(internals.Run.settings.dirPath, workspace), function (afterCommit) {
...
internals.finalizeFinishRun = function (err, jobId, pr, runId, finishConfig, jobConfig, pail) {
    ...
    var testFile = internals.Run.settings.getTestResult(jobId, runId);
    if (testFile) {
        //finishConfig.testFile = JSON.parse(testFile);
        finishConfig.testFile = testFile;
        //console.log(finishConfig.testFile);
    }
    if (err) {
        if (err.match('signal')) {
...
internals.runCommands = function (jobId, pr, runId, commands, cb) {
    ...
    internals.iterateCommands(jobId, pr, runId, commands, 0, function (err) {
        ...
        internals.finishRun(jobId, pr, runId, err, function () {
...
internals.iterateCommands = function (jobId, pr, runId, commands, cmdIndex, cb) {
    ...
    internals.runCommand(jobId, pr, runId, cmdIndex, nextCommand, function (result) {
        ...
        // hack for node 0.10 where npm doesnt actually send a signal when process.kill is done
//        if (result.code === 143 && result.command.match('^npm')) {
