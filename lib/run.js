...
exports.createRun = function (jobId, pr, cmds) {
    ...
    var pail;
    if (pr) {
        pail = new Pail({ dirPath: Path.join(internals.Run.settings.dirPath, jobId, 'pr', pr.number.toString()) });
    } else {
        pail = new Pail({ dirPath: Path.join(internals.Run.settings.dirPath, jobId) });
    }
...
exports.archiveArtifacts = function (jobId, runId) {
    ...
    if (job.archive) {
        ...
        pail.copyArtifact(internals.Run.settings.workspace, Path.join(runId, internals.Run.settings.archive), job.archive.pattern);
    }
};
       ...
       //if (jobConfig.scm.type === 'git') {
        ...
        if (pr) {
            if (internals.Run.settings.mock) {
                result = bobber.checkoutCode(Path.join(internals.Run.settings.dirPath, workspace), jobConfig.scm, null);
            } else {
                result = bobber.checkoutCode(Path.join(internals.Run.settings.dirPath, workspace), jobConfig.scm, pr);
            }
        } else {
            result = bobber.checkoutCode(Path.join(internals.Run.settings.dirPath, workspace), jobConfig.scm, null);
        }
        ...
        if (jobConfig.scm.runOnCommit) {
            ...
            var afterCommit = bobber.getLatestCommit(Path.join(internals.Run.settings.dirPath, workspace));
...
//     }
    ...
    if (finishConfig.status !== 'succeeded') {
        //console.log('notify not success');
        if (jobConfig.notify && jobConfig.notify.type === 'email') {
            var subject = jobConfig.notify.subject
                          .replace('{status}', finishConfig.status)
                          .replace('{subjectHeader}', internals.Run.settings.notify.plugins.email.options.subjectHeader)
                          .replace('{jobName}', jobConfig.name);
            var notifyOptions = {
                to: jobConfig.notify.to,
                subject: subject,
                message: jobConfig.notify.message
            };
