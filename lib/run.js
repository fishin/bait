...
exports.archiveArtifacts = function (jobId, runId) {
    ...
    if (job.archive) {
        ...
        var files = job.archive.pattern.trim().split(',');
...
internals.runCommand = function (jobId, pr, runId, cmdIndex, cmd, callback) {
    ...
    smelt.runCommand(cmd, function (result) {

        if (pr) {
            //internals.prRuns[jobId].pids.push(result.pid);
            internals.prRuns[jobId].prs[pr.number].pids.push(result.pid);
        } else {
            internals.jobRuns[jobId].pids.push(result.pid);
        }
...
internals.runCommand = function (jobId, pr, runId, cmdIndex, cmd, callback) {
    ...
    var subProcess = Child.spawn(command.cmd, command.args);
