        ...
        if (jobConfig.scm.type === 'git') {
            ...
            if (pr) {
                if (internals.Run.settings.mock) {
                    result = bobber.checkoutCodeSync(Path.join(internals.Run.settings.dirPath, workspace), jobConfig.scm, null);
                } else {
                    result = bobber.checkoutCodeSync(Path.join(internals.Run.settings.dirPath, workspace), jobConfig.scm, pr);
                }
            } else {
                result = bobber.checkoutCodeSync(Path.join(internals.Run.settings.dirPath, workspace), jobConfig.scm, null);
            }
...
internals.onFinish = function (jobId, pr, runId, err) {
    ...
    var testFile = internals.Run.getArchiveArtifact(jobId, runId, 'lab.json');
    if (testFile) {
