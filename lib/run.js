    ...
    if (jobConfig.scm) {
        if (jobConfig.scm.type === 'git') {
            ...
            bobber.checkoutCode(options, function (result) {
                ...
                getRun.checkout = result;
                runConfig = runPail.updatePail(getRun);
                commands = internals.parseCommands(runConfig.commands);
...
internals.cleanPid = function (jobId, pr) {
...
internals.initializePids = function (jobId, pr, runId) {
...
internals.runCommand = function (jobId, pr, runId, cmdIndex, cmd, callback) {
    ...
    var pidsObj = internals.initializePids(jobId, pr, runId);
