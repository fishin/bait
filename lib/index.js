...
module.exports = internals.Bait = function (options) {
    ...
    internals.Bait.archiveArtifacts = exports.archiveArtifacts;
    internals.Bait.getCommits = exports.getCommits;
...
exports.getRuns = function (jobId) {
    ...
    for (var i = 0; i < runs.length; i++) {
        var run = internals.Bait.getRun(jobId, runs[i]);
        fullRuns.push(run);
    }
    // sort by createTime
...
exports.getArchiveArtifact = function (jobId, runId, artifact) {
   ...
   var pail = new Pail({ dirPath: internals.Bait.settings.dirPath + '/' + jobId + '/' + runId });
   //console.log('getting artifact: ' + artifact);
   var contents = pail.getArtifact(internals.Bait.settings.archive, artifact);
   return contents;
};
...
exports.getArchiveArtifacts = function (jobId, runId) {
   ...
   var pail = new Pail({ dirPath: internals.Bait.settings.dirPath + '/' + jobId + '/' + runId });
   var files = pail.getFiles(internals.Bait.settings.archive);
   return files;
};
...
exports.archiveArtifacts = function (jobId, runId) {
   ...
   var pail = new Pail({ dirPath: internals.Bait.settings.dirPath + '/' + jobId });
   var jobPail = internals.Bait.getJob(jobId);
   if (jobPail.archive) {
       //console.log('archiving files: ' + jobPail.archive.pattern);
       //console.log(jobPail);
       pail.createDir(runId + '/' + internals.Bait.settings.archive);
       // handle more than 1 but for now...
       pail.copyArtifact(internals.Bait.settings.workspace, runId + '/' + internals.Bait.settings.archive, jobPail.archive.pattern);
   }
};
...
internals.startRun = function(jobId) {
    ...
    jobPail.createDir(jobId + '/' + internals.Bait.settings.workspace);
...
internals.onFinish = function (jobId, runId, err) {
    ...
    if (err) {
       ...
       updateConfig = pail.updatePail(finishConfig);
        ...
        //console.log('archiving artifacts due to success');
        internals.Bait.archiveArtifacts(jobId, runId);
    }
};
...
internals.runCommand = function (jobId, runId, cmdIndex, cmd, callback) {
    ...
    subProcess.on('error', function (err) {
        ...
        // may need logic here to check with pid to pop 
...
exports.getCommits = function(jobId, cb) {
    ...
    if (jobConfig.scm) {
       ...
       if (jobConfig.scm.type === 'git') {
           var bobber = new Bobber;
           var workspace = internals.Bait.settings.dirPath + '/' + jobId + '/' + internals.Bait.settings.workspace;
           bobber.getCommits(workspace, null, function(commits) {
...
exports.getTestResult = function(jobId, runId, artifact) {
    ...
    var hatchery = new Hatchery({ dirPath: internals.Bait.settings.dirPath + '/' + jobId + '/' + runId });
    var result = hatchery.getTestResult(internals.Bait.settings.archive, artifact);
    return result;
};
