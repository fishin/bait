...
//       if (jobConfig.scm.type === 'git') {
        ...
        var commits = bobber.getAllCommitsSync(workspace);
        return commits;
//       } else {
//           return [];
//       }
...
//       if (jobConfig.scm.type === 'git') {
        var bobber = new Bobber({ github: { url: internals.SCM.settings.github.url } });
        var workspace = Path.join(internals.SCM.settings.dirPath, jobId, internals.SCM.settings.workspace);
        var commit = bobber.getLatestCommitSync(workspace);
        return commit;
//       } else {
//           return null;
//       }
...
//       if (jobConfig.scm.type === 'git') {
        var bobber = new Bobber({ github: { url: internals.SCM.settings.github.url } });
        var commit = bobber.getLatestRemoteCommitSync(jobConfig.scm);
        return commit;
//       } else {
//           return null;
//       }
...
//       if (jobConfig.scm.type === 'git') {
        var bobber = new Bobber({ github: { url: internals.SCM.settings.github.url } });
        var workspace = Path.join(internals.SCM.settings.dirPath, jobId, internals.SCM.settings.workspace);
        var commits = bobber.getCompareCommitsSync(workspace, startCommit, endCommit);
        return commits;
//       } else {
//           return [];
//       }
